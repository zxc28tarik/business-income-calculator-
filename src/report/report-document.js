...
function renderCashFlow(report) {
  ...
  return `<section><div class="section-title"><div><p class="section-kicker">Likidite</p><h2>Açılış + 12 aylık birleşik nakit görünümü</h2></div></div>
...
function renderSetupSummary(setup) {
  const bridge = setup.cashBridge;
  const debt = setup.debtService;
  const totals = bridge.summary.totals;
  return `<div class="setup-summary">
    ${cashSummaryCard("Güvenli başlangıç nakdi", bridge.grossStartupCashNeed)}
    ${cashSummaryCard("Gerekli özkaynak", bridge.requiredOwnCash, bridge.requiredOwnCash > 0 ? "negative" : "positive")}
    ${cashSummaryCard("Hazır finansman", bridge.availableFunding, "positive")}
    ${cashSummaryCard("Planlanan finansman", setup.fundingSummary.plannedAmount)}
    ${cashSummaryCard("Varlık + stok + bağlı nakit", totals.assetBasis + totals.inventoryBasis + totals.tiedCash)}
    ${cashSummaryCard("12 ay sonu kalan borç", debt.endingBalance, debt.endingBalance > 0 ? "negative" : "positive")}
  </div>`;
}

function renderSetupItems(setup) {
  const items = setup.items ?? [];
  return `<details class="setup-details" open><summary>Kuruluş ve açılış kalemleri <span>${items.length} kalem</span></summary><div class="table-scroll"><table>
    <thead><tr><th>Durum</th><th>Kalem</th><th>Sınıf</th><th>Adet</th><th>Net</th><th>KDV</th><th>Nakit</th><th>Ödeme ayı</th><th>Taksit</th></tr></thead>
    <tbody>${items.length ? items.map((item) => `<tr><td>${escapeHtml(item.status)}</td><td>${escapeHtml(item.label)}</td><td>${escapeHtml(item.costType)}</td><td>${formatCell(item.quantity, "number")}</td><td>${formatCell(item.netAmount, "money")}</td><td>${formatCell(item.vatAmount, "money")}</td><td>${formatCell(item.cashAmount, "money")}</td><td>${formatCell(item.paymentMonth, "number")}</td><td>${formatCell(item.installmentCount, "number")}</td></tr>`).join("") : '<tr><td colspan="9">Kuruluş kalemi yok</td></tr>'}</tbody>
  </table></div></details>`;
}

function renderSetupFunding(setup) {
  const funding = setup.funding ?? [];
  return `<details class="setup-details" open><summary>Finansman ve borç koşulları <span>${funding.length} kaynak</span></summary><div class="table-scroll"><table>
    <thead><tr><th>Durum</th><th>Kaynak</th><th>Tür</th><th>Tutar</th><th>Hazır ay</th><th>Faiz</th><th>Vade</th><th>Ödemesiz</th><th>Masraf</th><th>Yöntem</th></tr></thead>
    <tbody>${funding.length ? funding.map((item) => `<tr><td>${escapeHtml(item.status)}</td><td>${escapeHtml(item.label)}</td><td>${escapeHtml(item.type)}</td><td>${formatCell(item.amount, "money")}</td><td>${formatCell(item.availableMonth, "number")}</td><td>${formatCell(item.annualInterestRate ?? 0, "percent")}</td><td>${formatCell(item.termMonths ?? 0, "number")}</td><td>${formatCell(item.graceMonths ?? 0, "number")}</td><td>${formatCell(item.upfrontFeeRate ?? 0, "percent")}</td><td>${escapeHtml(item.repaymentMethod ?? "")}</td></tr>`).join("") : '<tr><td colspan="10">Finansman kaynağı yok</td></tr>'}</tbody>
  </table></div></details>`;
}

function renderDebtSchedule(setup) {
  const integrated = setup.integratedCashFlow;
  return `<details class="setup-details"><summary>Kuruluş ve borç ödeme takvimi <span>${integrated.rows.length} dönem</span></summary><div class="table-scroll"><table>
    <thead><tr><th>Ay</th><th>Finansman girişi</th><th>Kuruluş ödemesi</th><th>Anapara</th><th>Faiz</th><th>Masraf</th><th>Net etki</th><th>Kalan borç</th></tr></thead>
    <tbody>${integrated.rows.map((row) => `<tr><td>${row.month === 0 ? "Açılış" : row.month}</td><td>${formatCell(row.setupFundingInflow, "money")}</td><td>${formatCell(row.setupPaymentOutflow, "money")}</td><td>${formatCell(row.debtPrincipalPayment, "money")}</td><td>${formatCell(row.debtInterestPayment, "money")}</td><td>${formatCell(row.debtFeePayment, "money")}</td><td>${formatCell(row.setupNetCashFlow, "money")}</td><td>${formatCell(row.endingDebtBalance, "money")}</td></tr>`).join("")}</tbody>
  </table></div></details>`;
}
...
function renderSetup(report) {
  ...
  if (!setup) return "";
  return `<section class="setup-report"><div class="section-title"><div><p class="section-kicker">Kuruluş · açılış · finansman</p><h2>İşletmeyi açmanın gerçek maliyeti</h2></div><span>${setup.unresolvedCount} doğrulama / teklif bekliyor</span></div>
    ${renderSetupSummary(setup)}
    ${renderSetupItems(setup)}
    ${renderSetupFunding(setup)}
    ${renderDebtSchedule(setup)}
  </section>`;
}

const REPORT_CSS = `
:root{font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;color:#17221d;background:#f3f5f4;font-synthesis:none;--ink:#17221d;--soft:#5e6b64;--line:#d8dfdb;--brand:#174a35;--brand-soft:#e5f0ea;--positive:#17603b;--positive-soft:#e4f3ea;--warning:#8a5b08;--warning-soft:#fff2cf;--danger:#a2342d;--danger-soft:#fbe8e5;--info:#355f7a;--info-soft:#e8f0f5}*{box-sizing:border-box}body{margin:0}.toolbar{position:sticky;top:0;z-index:5;display:flex;justify-content:flex-end;padding:12px 24px;background:var(--ink)}.toolbar button{min-height:44px;border:1px solid #fff;background:#fff;color:var(--brand);border-radius:8px;padding:9px 14px;font-weight:800;cursor:pointer}.report{max-width:1120px;margin:24px auto;background:#fff;padding:42px;border:1px solid var(--line);border-radius:14px;box-shadow:0 18px 50px rgba(24,32,29,.1)}header{border-bottom:3px solid var(--brand);padding-bottom:24px;margin-bottom:26px}.eyebrow,.section-kicker{margin:0;font-size:11px;font-weight:850;letter-spacing:.12em;text-transform:uppercase;color:var(--brand)}h1{font-size:34px;margin:6px 0 8px}h2{font-size:21px;margin:0}h3{font-size:16px;margin:0 0 12px}h4{margin:16px 0 8px}.meta{display:flex;flex-wrap:wrap;gap:10px 24px;color:var(--soft)}section{margin-top:32px}.section-title{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;margin-bottom:14px;padding-bottom:9px;border-bottom:1px solid var(--line)}.section-title>div{display:grid;gap:4px}.section-title>span{color:var(--soft);font-size:12px}.decision{margin:22px 0 0;padding:18px;border-radius:12px;border-left:7px solid #6b7b72;background:#f4f6f3}.decision.hard{border-color:var(--danger);background:var(--danger-soft)}.decision.soft{border-color:var(--warning);background:var(--warning-soft)}.decision.positive{border-color:var(--positive);background:var(--positive-soft)}.decision strong{font-size:20px}.decision-message{margin:8px 0 0;line-height:1.55}.decision-facts{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin:14px 0 0}.decision-facts div{padding:10px;border:1px solid rgba(23,34,29,.12);border-radius:8px;background:rgba(255,255,255,.65)}.decision-facts span,.decision-facts b{display:block}.decision-facts span{color:var(--soft);font-size:11px}.decision-facts b{margin-top:4px;font-size:14px}.summary{margin:10px 0 0;padding-left:20px;line-height:1.6}.kpi-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.primary-kpis{grid-template-columns:repeat(4,minmax(0,1fr))}.kpi{border:1px solid var(--line);border-radius:10px;padding:14px;background:#fafbfa}.kpi.primary{background:#fff}.kpi.negative{border-color:#e0a1a1;background:var(--danger-soft)}.kpi.positive strong{color:var(--positive)}.kpi span,.kpi small{display:block;color:var(--soft)}.kpi strong{display:block;font-size:20px;margin:6px 0;font-variant-numeric:tabular-nums}.secondary-kpis,.assumption-group,.setup-details{margin-top:12px;border:1px solid var(--line);border-radius:10px;overflow:hidden}.secondary-kpis summary,.assumption-group summary,.setup-details summary{display:flex;justify-content:space-between;gap:12px;padding:12px 14px;background:#f7f9f8;cursor:pointer;font-weight:800}.secondary-kpis summary span,.assumption-group summary span,.setup-details summary span{color:var(--soft);font-size:11px}.secondary-kpis .kpi-grid,.assumption-body{padding:14px}.warnings{display:grid;gap:9px}.warning{display:grid;grid-template-columns:190px 1fr;gap:14px;border:1px solid var(--line);border-left-width:4px;border-radius:9px;padding:12px 14px;background:#f4f6f3}.warning div{display:grid;gap:4px}.warning b{font-size:10px;text-transform:uppercase;letter-spacing:.06em}.warning strong{font-size:13px}.warning p{margin:0;line-height:1.5}.warning.critical{background:var(--danger-soft);border-left-color:var(--danger);color:#7e2020}.warning.warning{background:var(--warning-soft);border-left-color:var(--warning);color:#79500c}.warning.info{background:var(--info-soft);border-left-color:var(--info);color:var(--info)}.warning.positive{background:var(--positive-soft);border-left-color:var(--positive);color:var(--positive)}.split{border:1px solid var(--line);border-radius:10px;overflow:hidden}.split>div,.assumption-row{display:flex;justify-content:space-between;gap:20px;padding:10px 13px;border-bottom:1px solid #edf0ed}.split>div:last-child,.assumption-row:last-child{border-bottom:0}.assumption-table{margin-top:12px}.cash-summary,.setup-summary{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:14px}.setup-summary{grid-template-columns:repeat(3,1fr)}.cash-card{padding:14px;border:1px solid var(--line);border-radius:9px;background:#f7f9f8}.cash-card span,.cash-card strong{display:block}.cash-card span{color:var(--soft);font-size:12px}.cash-card strong{margin-top:6px;font-size:17px;font-variant-numeric:tabular-nums}.cash-card.negative{background:var(--danger-soft);border-color:#e0a1a1}.cash-card.negative strong,.negative-cash{color:var(--danger)}.cash-card.positive strong{color:var(--positive)}.table-scroll{overflow:auto;border:1px solid var(--line);border-radius:9px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{text-align:right;padding:9px 10px;border-bottom:1px solid #edf0ed;white-space:nowrap;font-variant-numeric:tabular-nums}th:first-child,td:first-child{text-align:left}th{background:#f3f5f3;color:#425148}.expected{background:var(--brand-soft)!important}.negative-cash{background:var(--danger-soft);font-weight:800}.disclaimer{margin-top:34px;padding:14px;border:1px solid var(--line);border-radius:10px;color:var(--soft);font-size:12px;line-height:1.5}@media(max-width:900px){.primary-kpis,.decision-facts,.cash-summary,.setup-summary{grid-template-columns:1fr 1fr}}@media(max-width:760px){.report{margin:0;padding:22px;border:0;border-radius:0}.kpi-grid,.primary-kpis,.decision-facts,.cash-summary,.setup-summary{grid-template-columns:1fr}.toolbar{position:static}.warning{grid-template-columns:1fr}}@page{size:A4;margin:14mm}@media print{body{background:#fff}.toolbar{display:none}.report{max-width:none;margin:0;padding:0;border:0;box-shadow:none}.secondary-kpis,.assumption-group,.setup-details{break-inside:avoid}.secondary-kpis>div,.assumption-group>.assumption-body{display:block!important}.kpi,.warning,.table-scroll,.cash-card{break-inside:avoid}section{break-inside:auto}h2{break-after:avoid}.primary-kpis{grid-template-columns:repeat(4,1fr)}.kpi-grid{grid-template-columns:repeat(3,1fr)}.cash-summary,.decision-facts{grid-template-columns:repeat(4,1fr)}.setup-summary{grid-template-columns:repeat(3,1fr)}}
`;
...
export function buildFinancialReportHtml(report) {
  ...
  <main class="report"><header><p class="eyebrow">BUSINESS INCOME CALCULATOR · FİNANSAL FİZİBİLİTE RAPORU</p><h1>${escapeHtml(report.sector.name)}</h1><div class="meta"><span>${escapeHtml(report.sector.family)}</span><span>${escapeHtml(subtitle)}</span><span>Motor ${escapeHtml(report.sector.version)}</span><span>Rapor ${escapeHtml(report.reportVersion)}</span><span>${escapeHtml(formatDate(report.generatedAt))}</span></div>
  <div class="decision ${escapeHtml(report.decision.tone)}"><strong>${escapeHtml(report.decision.label)}</strong><p class="decision-message">${escapeHtml(decisionMessage)}</p><div class="decision-facts"><div><span>Aylık net</span><b>${formatCell(report.decision.netProfit, "money")}</b></div><div><span>Birleşik 12 ay nakit</span><b>${formatCell(report.decision.endingCash, "money")}</b></div><div><span>Başabaş</span><b>${escapeHtml(report.decision.breakevenStatus ?? "Belirsiz")}</b></div><div><span>Kritik risk</span><b>${formatCell(report.decision.hardCount, "number")}</b></div></div><ul class="summary">${report.executiveSummary.map((sentence) => `<li>${escapeHtml(sentence)}</li>`).join("")}</ul></div></header>
  ${renderKpis(report)}${renderWarnings(report)}${renderSetup(report)}${renderScenarioTable(report)}${renderKeySplit(report)}${renderCashFlow(report)}${renderAssumptions(report)}
