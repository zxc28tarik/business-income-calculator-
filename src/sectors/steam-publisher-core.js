...
import { buildSteamBusinessProfileWarnings } from "./steam-business-profile-presentation.js";
...
export function buildSteamPublisherWarnings(result) {
  ...
  if (input.regionMode) {
    ...
    if (Math.abs(regionShareTotal - 1) > 0.005) add("region_share_total", "hard", "Bölge satış paylarının toplamı %100 olmalıdır.");
  }
  if (input.ipShareRate + input.coPublisherShareRate > 0.50) add("secondary_share_high", "soft", "IP ve co-publisher paylarının toplamı %50’yi aşıyor.");
  if (pnl.earningsBeforeTaxTry < 0) add("publisher_loss", "hard", "Bu senaryoda yayıncı vergi öncesi zarar ediyor.");
  if (settlement.unrecoupedExpenseTry > 0) add("recoup_open", "soft", "Recoup havuzunun bir bölümü kapanmıyor.");
  if (settlement.unrecoupedAdvanceTry > 0) add("advance_open", "soft", "Advance veya milestone tutarının bir bölümü geri alınamıyor.");
  if (receipt.receiptTry < 0) add("negative_receipt", "hard", "Banka ve tahsilat masrafları platform ödemesini aşıyor.");
  if (cashFlow.firstThreeMonthGapTry < 0) add("cash_gap", "soft", "İlk üç ayda ek finansman gerektirebilecek nakit açığı oluşuyor.");
  if (platform.adjustedGross <= 0) add("no_adjusted_revenue", "hard", "Vergi, iade ve ters ibraz sonrasında paylaşılabilir gelir oluşmuyor.");
  if (recoup.recoupableTotalTry > receipt.receiptTry && input.recoupEnabled) add("recoup_larger_than_receipt", "soft", "Recoup havuzu net yayıncı tahsilatından yüksek.");
...
export function buildSteamPublisherWaterfall(result) {
  ...
  return [
    { name: "Brüt oyuncu harcaması", amount: platform.customerPayment * usdTry, kind: "keep", subtext: "Vergiler dahil müşterinin ödediği" },
    ...
    { name: "Doğrudan ürün giderleri", amount: -recoup.directGameCostsTry, kind: "cut", subtext: "Recoupable ve non-recoupable toplam" },
...
export function calculateSteamPublisherReferenceModel(rawInputs = {}) {
  const prepared = buildSteamBusinessProfileInput(rawInputs);
  const input = normalizeSteamPublisherInputs(prepared.engineInput);
  ...
  const publisherRoi = totalPublisherOutlayTry > 0 ? result.tax.publisherNetProfitTry / totalPublisherOutlayTry : 0;
  const enriched = {
    ...result,
    input,
    sourceInput: prepared.sourceInput,
    profile: prepared.profile,
    profileMetrics: prepared.metrics,
    breakevenUnits,
    publisherRoi,
  };
  ...
  return {
    ...enriched,
    warnings: [...buildSteamPublisherWarnings(enriched), ...buildSteamBusinessProfileWarnings(enriched)],
    waterfall: buildSteamPublisherWaterfall(enriched),
  };
}
...
export function calculateSteamPublisherScenarioComparison(baseInputs = {}) {
  return Object.entries(STEAM_PUBLISHER_SCENARIOS).map(([id, scenario]) => {
    const masterScenarioInput = applySteamPublisherScenario(baseInputs, id);
    const input = applySteamBusinessProfileScenario(masterScenarioInput, id);
