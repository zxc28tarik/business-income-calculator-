...
test("v0.24.2 sürüm, production ve Pages sözleşmesi eşleşir", async () => {
  ...
  assert.equal(lock.version, "0.24.2");
  assert.equal(lock.packages[""].version, "0.24.2");
  assert.match(indexHtml, /BUSINESS INCOME CALCULATOR · v0\.24\.2/);
  assert.doesNotMatch(indexHtml, /BUSINESS INCOME CALCULATOR · v0\.23\.0/);
  assert.match(standaloneBuilder, /const APP_VERSION = "0\.24\.2"/);
